...
void wren_ConditionalTargetGetConditions(WrenVM* vm)
{
	...
	{
		WrenCondition* wrenCondition =
		    static_cast<WrenCondition*>(wrenSetSlotNewForeign(vm, 1, 2, sizeof(WrenCondition)));
...
void wren_ConditionGetCallback(WrenVM* vm)
{
	...
	ASSERT_MSG(callback, "condition callback %s not found, likley not registered",
	           wrenCondition->condition->GetKey().c_str());
...
void wren_CommandGetCallback(WrenVM* vm)
{
	...
	ASSERT_MSG(callback, "command callback %s not found, likley not registered",
	           wrenCommand->command->GetKey().c_str());
	wrenSetSlotCallback(vm, 0, callback);
}
...
void wren_DialogueNodeGetTargets(WrenVM* vm)
{
	...
	for (int i = 0; i < targetsCount; i++)
	{
		...
		WrenConditionalTarget* target =
		    static_cast<WrenConditionalTarget*>(wrenSetSlotNewForeign(vm, 1, 2, sizeof(WrenConditionalTarget)));
...
WREN_BINDING_MODULE(Dialogue)
{
	...
	WREN_FOREIGN_CLASS(registry, "dialogue", "DialogueResult", wren_DialogueResultAllocate, wren_DialogueResultFinalize,
	                   "Container for dialogue Result data");

	WREN_CONSTRUCTOR(registry, "dialogue", "DialogueResult", "nodeNotFound(_)", wren_DialogueResultNodeNotFound,
	                 "Node not found result");
	WREN_CONSTRUCTOR(registry, "dialogue", "DialogueResult", "endDialogue(_)", wren_DialogueResultEndDialogue,
	                 "End dialogue result");
	WREN_CONSTRUCTOR(registry, "dialogue", "DialogueResult", "choices(_)", wren_DialogueResultChoices,
	                 "Choices result");
	WREN_CONSTRUCTOR(registry, "dialogue", "DialogueResult", "advance(_)", wren_DialogueResultAdvance,
	                 "Advance result");
	WREN_CONSTRUCTOR(registry, "dialogue", "DialogueResult", "invalidChoice()", wren_DialogueResultInvalidChoice,
	                 "Invalid choice result");
	WREN_CONSTRUCTOR(registry, "dialogue", "DialogueResult", "noActiveNode()", wren_DialogueResultNoActiveNode,
	                 "No active node result");
	WREN_CONSTRUCTOR(registry, "dialogue", "DialogueResult", "error()", wren_DialogueResultError, "Error result");

	WREN_CLASS_METHOD(registry, "dialogue", "DialogueResult", "status", wren_DialogueResultGetStatus,
	                  "Get the status of the dialogue result");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueResult", "nodeId", wren_DialogueResultGetNodeId,
	                  "Get of the node id");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueResult", "speaker", wren_DialogueResultGetSpeaker,
	                  "Get the speaker of the node");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueResult", "text", wren_DialogueResultGetText,
	                  "Get the text of the node");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueResult", "choices", wren_DialogueResultGetChoices,
	                  "Get the text choices");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueResult", "hasEnded", wren_DialogueResultGetHasEnded,
	                  "Get the text has ended");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueResult", "shouldAutoAdvance",
	                  wren_DialogueResultGetShouldAutoAdvance, "Get the text should auto advance");

	// DialogueData foreign class
	WREN_FOREIGN_CLASS(registry, "dialogue", "ConditionalTarget", wren_ConditionalTargetAllocate,
	                   wren_ConditionalTargetFinalize, "Container for dialogue conditional target data");

	WREN_CLASS_METHOD(registry, "dialogue", "ConditionalTarget", "hasConditions()", wren_ConditionalTargetHasConditions,
	                  "Does the Target have conditions?");
	WREN_CLASS_METHOD(registry, "dialogue", "ConditionalTarget", "conditions", wren_ConditionalTargetGetConditions,
	                  "Get the conditions of the Conditional Target");
	WREN_CLASS_METHOD(registry, "dialogue", "ConditionalTarget", "targetNode", wren_ConditionalTargetGetTargetNode,
	                  "Get the target node of the Conditional Target");

	// DialogueData foreign class
	WREN_FOREIGN_CLASS(registry, "dialogue", "Condition", wren_ConditionAllocate, wren_ConditionFinalize,
	                   "Container for dialogue condition data");

	WREN_CLASS_METHOD(registry, "dialogue", "Condition", "callback", wren_ConditionGetCallback,
	                  "Get the callback handle");
	WREN_CLASS_METHOD(registry, "dialogue", "Condition", "params", wren_ConditionGetParams,
	                  "Get the conditions params");

	// DialogueData foreign class
	WREN_FOREIGN_CLASS(registry, "dialogue", "Command", wren_CommandAllocate, wren_CommandFinalize,
	...
	// DialogueData foreign class
	WREN_FOREIGN_CLASS(registry, "dialogue", "Choice", wren_ChoiceAllocate, wren_ChoiceFinalize,
	                   "Container for dialogue chioce data");

	WREN_CLASS_METHOD(registry, "dialogue", "Choice", "text", wren_ChoiceGetText, "Get the text");
	WREN_CLASS_METHOD(registry, "dialogue", "Choice", "targetNodeId", wren_ChoiceGetTargetNode,
	                  "Get the target node id");

	// DialogueData foreign class
	WREN_FOREIGN_CLASS(registry, "dialogue", "DialogueNode", wren_DialogueNodeAllocate, wren_DialogueNodeFinalize,
	...
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueNode", "speaker", wren_DialogueNodeGetSpeaker,
	                  "Get the speaker of the node");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueNode", "text", wren_DialogueNodeGetText,
	                  "Get the text of the node");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueNode", "commands", wren_DialogueNodeGetCommands,
	                  "Get the commands of the node");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueNode", "choices", wren_DialogueNodeGetChoices,
	...
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueNode", "targets", wren_DialogueNodeGetTargets,
	                  "Get the conditional targets of the node");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueNode", "hasChoices()", wren_DialogueNodeHasChoices,
	                  "Check if node has choices");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueNode", "hasNext()", wren_DialogueNodeGetHasNext,
	                  "Check if node has next target");
	WREN_CLASS_METHOD(registry, "dialogue", "DialogueNode", "hasTargets()", wren_DialogueNodeHasTargets,
	                  "Check if node has conditional targets");

	// DialogueRegistry static methods
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueRegistry", "registerCondition(_,_)",
	                  wren_DialogueRegistryRegisterCondition, "Register a condition type with callback");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueRegistry", "registerCommand(_,_)",
	                  wren_DialogueRegistryRegisterCommand, "Register a command type with callback");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueRegistry", "registerVariable(_,_)",
	                  wren_DialogueRegistryRegisterVariable, "Register a variable type with callback");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueRegistry", "getRegisteredVariable(_)",
	                  wren_DialogueRegistryGetRegisteredVariable, "Get the call back for a registered variable");

	// DialogueManager static methods
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueManager", "isDialogueActive()",
	                  wren_DialogueManagerIsDialogueActive, "Check if dialogue is active");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueManager", "getCurrentNodeId()",
	                  wren_DialogueManagerGetCurrentNodeId, "Get current node ID");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueManager", "getNodeCount()", wren_DialogueManagerGetNodeCount,
	                  "Get total number of loaded nodes");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueManager", "clearAllNodes()", wren_DialogueManagerClearAllNodes,
	                  "Clear all loaded dialogue nodes");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueManager", "loadDialogueData(_)",
	                  wren_DialogueManagerLoadDialogueData, "Loads in and interprets a wren map as dialogue");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueManager", "clearDialogue()", wren_DialogueManagerClearDialogue,
	                  "Clear the current dialogue interaction");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueManager", "currentNode", wren_DialogueManagerGetCurrentNode,
	                  "Get the current node in of the dialogue");
	WREN_CLASS_STATIC(registry, "dialogue", "DialogueManager", "setActiveNode(_)", wren_DialogueManagerSetActiveNode,
