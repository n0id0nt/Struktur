...
struct Choice
{
	...
	Choice(const std::string& t, const std::string& target)
	    : text(t),
	      targetNode(target)
	{
	}
...
class DialogueNode
{
   public:
	...
	const std::optional<std::string>& GetSpeaker() const
	{
		return m_speaker;
	}
	...
	const ChoiceList& GetChoices() const
	{
		return m_choices;
	}
	...
	const ConditionalTargetList& GetTargets() const
	{
		return m_targets;
	}
