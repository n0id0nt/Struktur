...
std::string VariableSubstitutionSystem::ApplyModifiers(GameContext& context, const DialogueValue& value,
                                                       const std::string& modifierChain)
{
	return ApplyModifierChain(context, value, modifierChain);
}

void VariableSubstitutionSystem::RegisterModifier(const std::string& name, std::unique_ptr<VariableModifier> modifier)
{
	m_modifiers[name] = std::move(modifier);
}

void VariableSubstitutionSystem::RegisterTemplate(const std::string& name, const VariableTemplate& templ)
{
	m_templates[name] = templ;
}

void VariableSubstitutionSystem::RegisterTemplate(const std::string& name, const std::vector<std::string>& modifiers)
{
...
std::string VariableSubstitutionSystem::ApplyModifierChain(GameContext& context, const DialogueValue& value,
...
std::string VariableSubstitutionSystem::ApplySingleModifier(GameContext& context, const DialogueValue& value,
                                                            const std::string& modifierStr) const
{
	// Parse modifier: "name:arg"
	size_t colonPos     = modifierStr.find(':');
	std::string modName = (colonPos != std::string::npos) ? modifierStr.substr(0, colonPos) : modifierStr;
	std::string modArg  = (colonPos != std::string::npos) ? modifierStr.substr(colonPos + 1) : "";
