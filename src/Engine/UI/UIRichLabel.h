...
//
// Deliberately NOT yet implemented: text alignment (this always renders left-aligned).
...
public:
	...
	// One quad per non-space/tab codepoint across every text run's text, plus one per icon run that actually
	// resolves against the current IconAtlas, summed over every line - the same "how many glyphs total" count
	// SetVisibleGlyphCount's cap is measured against. Independent of m_visibleGlyphCount itself - this is the
	// full count regardless of how much is currently set to actually render.
...
private:
	...
	// Writes one line's runs left-to-right starting at pos, into a sub-slot of m_batchSlot offset by
	// quadOffset quads (same offset-accumulation contract UILabel::RenderGlyphs/RenderJustifiedLine use).
	// remainingVisible is the reveal budget (see SetVisibleGlyphCount) - decremented per glyph/icon actually
	// drawn, shared across every line's call via the same reference so the budget carries over correctly from
	// one line to the next. Returns how many quads it actually wrote.
