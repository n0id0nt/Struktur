...
namespace Struktur::Input
{
		...
		, m_gamepadIndex(gamepadIndex)
	{
		if (IsGamepadAvailable(m_gamepadIndex))
		{
			...
			DEBUG_INFO(std::format("Gamepad {} '{}' successfully connected.",
				m_gamepadIndex, m_gamepadId).c_str());
		}
		else
		{
	...
	void Input::SetGamepadIndex(int index)
	{
		...
		{
			m_gamepadId = GetGamepadName(m_gamepadIndex);
			DEBUG_INFO(std::format("Switched to gamepad {} '{}'",
				m_gamepadIndex, m_gamepadId).c_str());
		}
		else
		{
	...
	{
		if (component == "positive") return AxisComponent::Positive;
		if (component == "negative") return AxisComponent::Negative;
	...
	{
		if (component == "up") return Axis2Component::Up;
		if (component == "down") return Axis2Component::Down;
		if (component == "left") return Axis2Component::Left;
		if (component == "right") return Axis2Component::Right;
	...
	{
		if (direction == "x") return Axis2Direction::X;
		if (direction == "y") return Axis2Direction::Y;
	...
	{
		if (axis == "positive") return VariableBindingAxis::Positive;
		if (axis == "negative") return VariableBindingAxis::Negative;
		if (axis == "negativeToPositive") return VariableBindingAxis::NegativeToPositive;
		if (axis == "positiveToNegative") return VariableBindingAxis::PositiveToNegative;
	...
	{
		return CheckBinding(input,
			[this](::KeyboardKey k) { return IsKeyDown(k); },
	...
	{
		return CheckBinding(input,
			[this](::KeyboardKey k) { return IsKeyJustPressed(k); },
	...
	{
		return CheckBinding(input,
			[this](::KeyboardKey k) { return IsKeyJustReleased(k); },
	...
	float Input::GetInputVariable(const std::string& input)
	{
		...
		ASSERT_MSG(it != m_variableBindings.end(),
			std::format("Variable binding '{}' not found", input).c_str());
		...
		{
			value += GetControllerVariableValue(
				controllerVar.controllerAxis,
				controllerVar.variableBindingAxis);
		}
	...
	float Input::GetInputAxis(const std::string& input)
	{
		...
		ASSERT_MSG(it != m_axisBindings.end(),
			std::format("Axis binding '{}' not found", input).c_str());
	...
	glm::vec2 Input::GetInputAxis2(const std::string& input)
	{
		...
		ASSERT_MSG(it != m_axis2Bindings.end(),
			std::format("Axis2 binding '{}' not found", input).c_str());
