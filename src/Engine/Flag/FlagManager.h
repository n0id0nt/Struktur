...
class FlagManager
{
   public:
	...
	// Debug/Inspection
	size_t GetBoolFlagCount() const
	{
		return m_boolFlags.size();
	}
	size_t GetIntFlagCount() const
	{
		return m_intFlags.size();
	}
	size_t GetFloatFlagCount() const
	{
		return m_floatFlags.size();
	}
	size_t GetStringFlagCount() const
	{
		return m_stringFlags.size();
	}

	const std::unordered_map<std::string, bool>& GetAllBoolFlags() const
	{
		return m_boolFlags;
	}
	const std::unordered_map<std::string, int>& GetAllIntFlags() const
	{
		return m_intFlags;
	}
	const std::unordered_map<std::string, float>& GetAllFloatFlags() const
	{
		return m_floatFlags;
	}
	const std::unordered_map<std::string, std::string>& GetAllStringFlags() const
	{
