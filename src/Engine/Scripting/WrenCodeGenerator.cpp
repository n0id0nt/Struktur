...
void CodeGenerator::GenerateModuleFile(const std::string& outputDir, const std::string& moduleName,
	...
	for (const auto& [className, classMethods] : methodsByClass)
	{
		...
		for (const auto* constant : constants)
		{
			if (constant->className == className)
			{
				...
				file << "    static " << constant->name << " { " << constant->value << " }\n";
			}
		}
...
void CodeGenerator::GenerateAllHeaders(const std::string& wrenSourceDir, const std::string& headerOutputDir)
{
	...
	DEBUG_INFO("Generated combined header with %zu module(s): %s", moduleNames.size(), combinedPath.c_str());
}
}  // namespace Struktur::Wren
