...
void Struktur::Wren::WrenScriptComponentRegistry::RegisterScriptComponent(std::string module, std::string className)
{
    ...
    if (!inserted)
    {
        DEBUG_ERROR("[WrenScriptComponentRegistry] Script %s: has already been added to wren script registy", className);
    }
}
...
void Struktur::Wren::WrenScriptComponentRegistry::Shutdown(GameContext &context)
{
    ...
    for (auto& it : m_scriptComponents)
    {
        ...
        if (scriptComponent.startMethodHandle) wrenReleaseHandle(vm, scriptComponent.classHandle);
        if (scriptComponent.updateMethodHandle) wrenReleaseHandle(vm, scriptComponent.classHandle);
        if (scriptComponent.onDestroyMethodHandle) wrenReleaseHandle(vm, scriptComponent.classHandle);
...
void Struktur::Wren::WrenScriptComponentRegistry::ReloadScript(GameContext &context)
{
    //// Destroy old script
