...
class FileSystem
{
   public:
	...
	static std::vector<std::string> OpenFileDialogMultiple(const std::string& title,
	...
	// Returns empty string if user cancels
	static std::string SaveFileDialog(const std::string& title, const std::string& defaultPath = "",
	                                  const std::vector<std::string>& filters = {}, const std::string& filterDesc = "");

	// Opens a native OS message box
	// okCancel = false : ok only
	// okCancel = true  : ok + cancel, returns true if user clicked ok
	static bool MessageBox(const std::string& title, const std::string& message, bool okCancel = false);
