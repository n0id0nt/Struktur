...
void Struktur::System::DebugSystem::Update(GameContext &context)
{
    ...
    if (debugSettings.showPhysicsShapes || 
        debugSettings.showPhysicsBodies || 
        debugSettings.showPhysicsJoints || 
        debugSettings.showPhysicsAABBs)
    {
        m_box2dRenderer.RenderWorld(
            physicsWorld.GetRawWorld(), 
            physicsWorld.GetPixelsPerMeter(),
