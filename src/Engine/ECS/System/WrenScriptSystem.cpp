...
void WrenScriptSystem::Update(GameContext& context)
{
    ...
    for (auto entity : view)
    {
        ...
        {
            DEBUG_WARNING("Script %s is not initialised - THIS IS HERE AS A TODO TO FIX PREVIOUS IMPLEMENTAITON OF INHERATENCE ONVE DONE MAKE THIS AN ERROR", script.className.c_str());
        ...
        if (result != WREN_RESULT_SUCCESS)
        {
            DEBUG_ERROR("Error calling Update() on entity %d script: %s", static_cast<int>(entity), script.className.c_str());
            script.hasError = true;
...
void WrenScriptSystem::SendEvent(GameContext& context, entt::entity entity, Component::WrenScript& script, const std::string& eventType, const std::unordered_map<std::string, double>& eventData)
{
