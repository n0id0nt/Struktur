...
namespace Struktur::Debug
{
    void SpritePreviewRenderer::Render(GameContext& context, const ImVec2& availableSize)
    {
        ...
        ImGui::Text("Color: RGBA(%d,%d,%d,%d)", 
            m_sprite->color.r, m_sprite->color.g, m_sprite->color.b, m_sprite->color.a);
        ImGui::Separator();
        ...
        Rectangle sourceRec = {
            col * frameWidth + m_sprite->offset.x,
            row * frameHeight + m_sprite->offset.y,
            m_sprite->flipped ? -frameWidth : frameWidth,
            frameHeight
        };
