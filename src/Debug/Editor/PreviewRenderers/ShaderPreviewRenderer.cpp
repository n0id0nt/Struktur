...
namespace Struktur::Debug
{
    ...
    void ShaderPreviewRenderer::RenderShaderPreview(const ImVec2& size)
    {
        ...
        // Begin shader mode
        BeginShaderMode(m_shader->shader);
        ...
        switch (m_previewShape)
        {
            case ShaderPreviewShape::Quad2D:
            {
                ...
                Rectangle dest = {
                    m_renderTexture.texture.width / 2.0f - 200,
                    m_renderTexture.texture.height / 2.0f - 200,
                    400, 400
                };
                Rectangle source = {0, 0, (float)m_previewTexture.width, (float)m_previewTexture.height};
                DrawTexturePro(m_previewTexture, source, dest, 
                    Vector2{200, 200}, m_rotation, WHITE);
                break;
            }
            ...
            case ShaderPreviewShape::Cube:
            {
                ...
                Rectangle dest = {
                    m_renderTexture.texture.width / 2.0f - 150,
                    m_renderTexture.texture.height / 2.0f - 150,
                    300, 300
                };
    ...
    void ShaderPreviewRenderer::ApplyShaderUniforms()
    {
        ...
        {
            int location = GetShaderLocation(m_shader->shader, name.c_str());
            if (location != -1)
                SetShaderValue(m_shader->shader, location, &value, SHADER_UNIFORM_FLOAT);
        }
        ...
        for (const auto& [name, value] : m_shader->intUniforms)
        {
            int location = GetShaderLocation(m_shader->shader, name.c_str());
            if (location != -1)
                SetShaderValue(m_shader->shader, location, &value, SHADER_UNIFORM_INT);
        }
        ...
        for (const auto& [name, value] : m_shader->vec2Uniforms)
        {
            int location = GetShaderLocation(m_shader->shader, name.c_str());
            if (location != -1)
                SetShaderValue(m_shader->shader, location, &value, SHADER_UNIFORM_VEC2);
        }
        ...
        for (const auto& [name, value] : m_shader->vec3Uniforms)
        {
            int location = GetShaderLocation(m_shader->shader, name.c_str());
            if (location != -1)
                SetShaderValue(m_shader->shader, location, &value, SHADER_UNIFORM_VEC3);
        }
        ...
        for (const auto& [name, value] : m_shader->vec4Uniforms)
        {
            int location = GetShaderLocation(m_shader->shader, name.c_str());
            if (location != -1)
                SetShaderValue(m_shader->shader, location, &value, SHADER_UNIFORM_VEC4);
        }
        ...
        for (const auto& [name, value] : m_shader->matrixUniforms)
        {
            int location = GetShaderLocation(m_shader->shader, name.c_str());
            if (location != -1)
                SetShaderValueMatrix(m_shader->shader, location, value);
        }
    }
}