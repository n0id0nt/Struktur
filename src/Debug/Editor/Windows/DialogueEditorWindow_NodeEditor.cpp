...
void DialogueEditorWindow::RenderNodeBasicInfo(GameContext& context, Dialogue::DialogueNode* node)
{
	...
	ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "💡 Choose how this dialogue node continues");
...
void DialogueEditorWindow::RenderNodeChoices(GameContext& context, Dialogue::DialogueNode* node)
{
	...
	ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "💡 Player will choose one of these options");
}
...
void DialogueEditorWindow::RenderNodeTargets(GameContext& context, Dialogue::DialogueNode* node)
{
	...
	for (size_t i = 0; i < targets.size(); ++i)
	{
		...
		if (isOpen)
		{
			...
			if (conditions.empty())
			{
				ImGui::TextColored(ImVec4(1.0f, 0.7f, 0.3f, 1.0f), "⚠️ No conditions - this target will always match");
			}
...
void DialogueEditorWindow::SwitchNodeContinuationType(Dialogue::DialogueNode* node, int newType)
{
	...
	const std::optional<std::string>& savedSpeaker = node->GetSpeaker();
	const std::optional<std::string>& savedText    = node->GetText();
