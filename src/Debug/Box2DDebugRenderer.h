...
class Box2DDebugRenderer
{
   private:
	...
	void DrawCircleShape(const b2CircleShape* circle, const b2Transform& transform, float scale, bool filled)
	{
		...
		::Vector2 radiusEnd = {screenCenter.x + radiusDirection.x * scale, screenCenter.y + radiusDirection.y * scale};
	...
	void DrawVelocityArrow(::b2Body* body, ::Color velocityColor, float velocityScale, float scale)
	{
		...
		::Vector2 velocityEnd   = {bodyCenter.x + scaledVelocity.x, bodyCenter.y + scaledVelocity.y};
	...
	void DrawAngularVelocity(::b2Body* body, ::Color angularColor, float radiusScale, float scale)
	{
		...
		for (int i = 0; i < segments; i++)
		{
			...
			::Vector2 point1 = {bodyCenter.x + ::cosf(angle1) * radius, bodyCenter.y + ::sinf(angle1) * radius};
			::Vector2 point2 = {bodyCenter.x + ::cosf(angle2) * radius, bodyCenter.y + ::sinf(angle2) * radius};
		...
		::Vector2 arcEnd = {bodyCenter.x + ::cosf(finalAngle) * radius, bodyCenter.y + ::sinf(finalAngle) * radius};
		...
		::Vector2 arrowTip = {arcEnd.x + arrowDir.x * arrowSize, arcEnd.y + arrowDir.y * arrowSize};

		::DrawLineEx(arcEnd, arrowTip, m_lineThickness, angularColor);
	}

	// Draw a single contact point with optional normal and impulse
	void DrawContactPoint(const glm::vec2& point, const glm::vec2& normal, float impulse, float scale, bool drawNormal,
	                      bool drawImpulse)
	{
		...
		::DrawCircleLines(screenPoint.x, screenPoint.y, m_contactSettings.pointRadius,
		                  ColorBrightness(m_contactSettings.pointColor, -0.3f));

		// Draw contact normal
		if (drawNormal && m_contactSettings.drawNormals)
		{
			Vector2 normalEnd = {screenPoint.x + normal.x * m_contactSettings.normalLength,
		...
		if (drawImpulse && m_contactSettings.drawImpulses && impulse > 0.01f)
		{
			...
			Vector2 impulseEnd      = {screenPoint.x + perpendicular.x * impulseLength,
   ...
   public:
	...
	void RenderWorld(::b2World* world, float scale = 32.0f, bool drawFilled = false, bool drawCenterOfMass = false,
	                 bool drawVelocity = false, bool drawAngularVelocity = false)
	{
	...
	void RenderBody(::b2Body* body, float scale = 32.0f, bool drawFilled = false, bool drawCenterOfMass = false,
	                bool drawVelocity = false, bool drawAngularVelocity = false)
	{
		if (!body)
		{
	...
	void RenderFixture(::b2Fixture* fixture, const ::b2Transform& transform, float scale = 32.0f,
	...
	void RenderContactPoints(const ContactContainer& contacts, float scale = 32.0f, bool drawNormals = true,
	                         bool drawImpulses = true)
	{
	...
	void RenderContact(const Physics::ContactListener::Contact& contact, float scale = 32.0f, bool drawNormals = true,
	                   bool drawImpulses = true)
	{
		if (!contact.enabled)
		{
	...
	void RenderBodiesIf(b2World& world, Predicate pred, float scale, bool drawFilled, bool drawCenterOfMass,
	                    bool drawVelocity, bool drawAngularVelocity)
	{
		for (b2Body* body = world.GetBodyList(); body != nullptr; body = body->GetNext())
		{
			if (pred(body))
	...
	void RenderDynamicBodies(b2World& world, float scale, bool drawFilled, bool drawCenterOfMass, bool drawVelocity,
	                         bool drawAngularVelocity)
	{
		RenderBodiesIf(
		    world, [](b2Body* body) { return body->GetType() == b2_dynamicBody; }, scale, drawFilled, drawCenterOfMass,
		    drawVelocity, drawAngularVelocity);
	}

	// Render only static bodies
	void RenderStaticBodies(b2World& world, float scale, bool drawFilled, bool drawCenterOfMass, bool drawVelocity,
	                        bool drawAngularVelocity)
	{
		RenderBodiesIf(
		    world, [](b2Body* body) { return body->GetType() == b2_staticBody; }, scale, drawFilled, drawCenterOfMass,
		    drawVelocity, drawAngularVelocity);
	}

	// Render only kinematic bodies
	void RenderKinematicBodies(b2World& world, float scale, bool drawFilled, bool drawCenterOfMass, bool drawVelocity,
	                           bool drawAngularVelocity)
	{
		RenderBodiesIf(
		    world, [](b2Body* body) { return body->GetType() == b2_kinematicBody; }, scale, drawFilled,
		    drawCenterOfMass, drawVelocity, drawAngularVelocity);
	}
	...
	// Contact point visualization settings
	void SetContactPointColor(Color color)
	{
		m_contactSettings.pointColor = color;
	}
	void SetContactNormalColor(Color color)
	{
		m_contactSettings.normalColor = color;
	}
	void SetContactImpulseColor(Color color)
	{
		m_contactSettings.impulseColor = color;
	}
	void SetContactPointRadius(float radius)
	{
		m_contactSettings.pointRadius = radius;
	}
	void SetContactNormalLength(float length)
	{
		m_contactSettings.normalLength = length;
	}
	void SetContactImpulseScale(float scale)
	{
		m_contactSettings.impulseScale = scale;
	}
	...
	const ContactPointSettings& GetContactSettings() const
	{
