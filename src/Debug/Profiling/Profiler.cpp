...
void Struktur::Debug::Profiler::EndProfile()
{
	...
	// For display purposes, store the total accumulated time in duration_us
	// This will be the sum of all calls to this function in the current frame
	m_currentNode->duration_us = m_currentNode->accumulated_duration_us;
...
{
	// First, recursively calculate self duration for all children
	for (auto child : node->children)
	{
		CalculateSelfDuration(child);
	}

	// Then calculate this node's self duration
	...
	// Sanity check - self duration shouldn't be negative
	if (node->self_duration_us < 0)
	{
		node->self_duration_us = 0;
	}
}