	...
	namespace Debug
	{
		struct ProfileNode
		{
			...
			ProfileNode(const std::string& n, size_t d, ProfileNode* p = nullptr)
				: name(n), duration_us(0), self_duration_us(0), accumulated_duration_us(0),
		...
		{
		public:
			ScopedProfile(Profiler* profiler, const std::string& name) : m_profiler(profiler)
			{
...
// Macros
#ifdef DEBUG
#define PROFILE_FUNCTION() Struktur::Debug::ScopedProfile profiler_##__LINE__(&(context.GetProfiler()), __FUNCTION__)
#define PROFILE_SCOPE(name) Struktur::Debug::ScopedProfile profiler_##__LINE__(&(context.GetProfiler()), name)
#define PROFILE_BEGIN_SCOPE(variable, name) Struktur::Debug::ScopedProfile profiler_##variable(&(context.GetProfiler()), name);
